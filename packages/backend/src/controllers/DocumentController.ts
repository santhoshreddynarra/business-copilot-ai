...
export class DocumentController {
  ...
  static async uploadDocument(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const userId = req.user?.id || 'mock-user-id'; // Fallback until auth is fully integrated
  ...
  static async getDocumentDetails(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const document = await documentService.getDocumentDetails(userId, id);
  ...
  static async downloadDocument(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { buffer, originalName, mimetype } = await documentService.getDownloadBuffer(userId, id);
